...
I have modified my program to make it fail test 5:

[[file:test5.png]]

To debug it there are several ways. The simplest is to run it from the command line and see what happens.
Make sure you are in the ~build~ directory and run:

#+BEGIN_SRC 
./readnumbers.exe < ../tests/test-05.out
#+END_SRC
...
What is the expected output? We can inspect the contents of the file ~tests/test-05.expected~:
...
We can compare the output of the two commands more easily. Make sure you run this from a cygwin64 terminal in the ~build~ directory:
